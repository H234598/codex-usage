...
def _write_private_service_runtime(
    ...
    dist_info = site_packages / "codex_usage-0.6.540.dist-info"
    for directory in (
    ...
    rows.extend(
        (
            (
                "codex_usage-0.6.540.dist-info/METADATA,"
                f"{_record_digest(metadata.read_bytes())},{metadata.stat().st_size}"
            ),
            "codex_usage-0.6.540.dist-info/RECORD,,",
        )
    )
...
def test_private_service_runtime_attestation_rejects_unsafe_layout_or_drift(
    ...
    dist_info = runtime_entrypoint.parent.parent / "codex_usage-0.6.540.dist-info"
...
def _write_trusted_runtime_core_layout(
    ...
    trusted_dist = trusted_site / "codex_usage-0.6.540.dist-info"
    ...
    trusted_rows.extend(
        [
            (
                f"codex_usage-0.6.540.dist-info/METADATA,"
    ...
    release_dir = tmp_path / "state/codex-usage/integration/releases/0.6.540-6666666666666666"
    ...
    active_dist = active_site / "codex_usage_integration_producer-0.6.540.dist-info"
    ...
    active_rows.extend(
        [
            (
                f"codex_usage_integration_producer-0.6.540.dist-info/{name},"
    ...
    active_rows.append("codex_usage_integration_producer-0.6.540.dist-info/RECORD,,")
...
def test_execute_rejects_nonprivate_service_dist_info_before_publisher(
    ...
    monkeypatch.setattr(integration_watchdog.sys, "executable", str(interpreter))
    _patch_private_service_runtime_origins(monkeypatch, runtime_entrypoint)
    runtime_entrypoint.parent.parent.joinpath("codex_usage-0.6.540.dist-info").chmod(
        0o777
    )
...
def test_execute_rejects_record_consistent_core_runtime_rebind_before_publisher(
    ...
    _patch_private_service_runtime_origins(monkeypatch, runtime_entrypoint)
    record = (
        runtime_entrypoint.parent.parent / "codex_usage-0.6.540.dist-info" / "RECORD"
    )
