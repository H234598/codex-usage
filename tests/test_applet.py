...
def test_applet_metadata_and_settings_are_consistent() -> None:
    ...
    assert settings["hide-account-when-long-limit-exhausted"]["type"] == "switch"
    assert settings["hide-account-when-long-limit-exhausted"]["default"] is False
    assert "hide-account-when-long-limit-exhausted" in settings["layout"]["display-section"]["keys"]
    ...
    assert panel_table["show-buttons"] is True
    assert panel_table["hidden-buttons"] == ["+", "-", "up", "down"]
    ...
    assert "von oben nach unten" in edit_columns["tooltip"]
    assert "automatisch" in edit_columns["tooltip"]
