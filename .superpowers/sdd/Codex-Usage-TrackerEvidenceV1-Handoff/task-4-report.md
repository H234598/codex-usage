...
korrigiert. Der danach vorgeschriebene erneute Installerlauf traf erwartbar
...
Active-Swap erhalten die vorherige Generation. Fehler nach erfolgreichem
`active.json`-Swap, aber vor Abschluss von finaler Attestierung und
`previous.json`-Fortschreibung, stellen die vorherigen Active-Bytes über die
bestehende sichere atomare Private-IO-Schreiblogik wieder her und prüfen sie
bytegenau nach. `previous.json` bleibt dabei bytegenau unverändert. Scheitert
die Rücksicherung selbst, bleibt das fehlgeschlagene Publish-Artefakt als
Evidenz liegen und der Installer liefert den begrenzten
`IntegrationCleanupError` statt eines stärkeren Workarounds.
...
Finales Ergebnis nach Round-1-Fix:
`576 passed, 1 warning in 111.45s`.
...
- Echte Post-Swap-Install- und Rollbackfehler bewahren `active.json` bytegenau
  und ändern `previous.json` nicht. Der neue Cleanup-Test beweist zusätzlich:
  scheitert die atomare Active-Rücksicherung selbst, folgt der eigene bounded
  Cleanupfehler, das fehlgeschlagene Manifest bleibt als Evidenz bestehen und
  Previous bleibt unberührt. Existierende umfassende Race-/Mode-/Owner-/Link-
  Tests blieben grün.
