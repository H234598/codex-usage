    ...
    *,
    ...
    try:
        ...
        status = (
            AccountStatus.OK
            if main_is_usable
            and (has_dynamic_usage or _has_usage_values(five_hour, weekly))
...
def _resolve_auth_json_path(account: Account, override: Path | None) -> Path:
    ...
    configured_path = account.auth_json_path
...
def canonical_backend_identity(
    ...
    *,
    ...
    for field, value, maximum in (
        ...
        ("auth_user_id", auth_user_id, MAX_AUTH_ID_CHARS),
        ("auth_account_id", auth_account_id, MAX_AUTH_ID_CHARS),
        ("auth_plan_type", auth_plan_type, 64),
...
def _expanded_auth_path(path: object) -> Path:
    try:
        return _require_auth_path(path).expanduser()
    except RuntimeError as exc:
        raise DirectAuthError("auth.json path is invalid") from exc
